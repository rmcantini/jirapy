...
# Construct the JQL query to search for issues with worklogs by your user updated within the day
jql_query = f"worklogAuthor = '{JIRA_USERNAME}' AND worklogDate >= startOfDay() AND worklogDate <= endOfDay()"
...
# Calculate the total time spent by your user between 9 am to 11 pm on the current day
for issue in issues:
...
print(
    f"Total Time Spent by {JIRA_USERNAME} on {current_date} between 9 am to 11 pm: {total_hours} hours and {total_minutes} minutes")
...
for idx, issue in enumerate(issues, start=1):
    for worklog in issue.fields.worklog.worklogs:
        ...
        if start_of_day <= worklog_date <= end_of_day:
            ...
            print(
                f"{idx}. Issue Key: {issue.key}, Time Spent: {time_spent // 3600}h {time_spent % 3600 // 60}m")

print(
    f"\nTotal Time Worked in the Day: {total_hours} hours and {total_minutes} minutes")
